...
public class JDialogForgetPassword extends JDialog {
	...
	private JButton jbuttonSetNewPassword;
	private JButton jbuttonCancel;
	private Map<String, Object> dataUsername;
	private JButton jbuttonResendCode;
	...
	public static void main(String[] args) {
		try {
			JDialogForgetPassword dialog = new JDialogForgetPassword();
	...
	public JDialogForgetPassword() {
		...
		}
		{
			...
			gbc_jpasswordFieldConfirm.gridy = 3;
		...
		{
			jbuttonResendCode = new JButton("Resend Code");
			jbuttonResendCode.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent e) {
					do_jbuttonResendCode_actionPerformed(e);
				}
			});
			GridBagConstraints gbc_jbuttonResendCode = new GridBagConstraints();
			gbc_jbuttonResendCode.insets = new Insets(0, 0, 5, 5);
			gbc_jbuttonResendCode.gridx = 3;
			gbc_jbuttonResendCode.gridy = 4;
			getContentPane().add(jbuttonResendCode, gbc_jbuttonResendCode);
		}
		{
			...
			{
				jbuttonSetNewPassword = new JButton("Set New Password");
				jbuttonSetNewPassword.addActionListener(new ActionListener() {
					public void actionPerformed(ActionEvent e) {
						do_jbuttonSetNewPassword_actionPerformed(e);
					}
				});
				panel.add(jbuttonSetNewPassword);
			}
			{
	...
	public JDialogForgetPassword(Map<String, Object> dataUsername) {
		this();
	...
	protected void do_jbuttonResendCode_actionPerformed(ActionEvent e) {
		EmployeeModel employeeModel = new EmployeeModel();
		String username = dataUsername.get("username").toString();
...
//				+ "<p>Verification Code: " + verificationCode + "</p>";
		...
		String mailBody = "<!--\r\n"
				...
				+ "</html>";
		String mailTo = employeeModel.findByUsername(username).getEmail();
		...
		if(MailHelpers.send("vinhduc0304@gmail.com", mailTo, mailSubject, mailBody)) {
			...
			JOptionPane.showMessageDialog(null, "Undefined error. Cannot resend verification code.", "System Message", JOptionPane.ERROR_MESSAGE);
		}
	}
	
	protected void do_jbuttonSetNewPassword_actionPerformed(ActionEvent e) {
		EmployeeModel employeeModel = new EmployeeModel();
		String username = dataUsername.get("username").toString();
		...
		if(confirmPw.isEmpty() || confirmPw == null) {
			JOptionPane.showMessageDialog(null, "Confirm password cannot be empty.", "System Message", JOptionPane.ERROR_MESSAGE);
			return;
		}
		String inputCodeString = jtextFieldVerificationCode.getText();			
		if(inputCodeString.isEmpty() || inputCodeString == null) {
			JOptionPane.showMessageDialog(null, "Verification code cannot be empty.", "System Message", JOptionPane.ERROR_MESSAGE);
			return;
		}
		
		if(!ValidateHelper.isNumber(inputCodeString)) {
			JOptionPane.showMessageDialog(null, "Please enter a number type verification code.", "System Message", JOptionPane.ERROR_MESSAGE);
			return;
		}
		
		Integer inputCode = Integer.parseInt(inputCodeString);
		int dbCode = employeeModel.findByUsername(username).getSecurityCode();
		if(inputCode != dbCode) {
			JOptionPane.showMessageDialog(null, "Incorrect verification code.", "System Message", JOptionPane.ERROR_MESSAGE);
			return;
		}
		if(!newPw.equals(confirmPw)) {
			JOptionPane.showMessageDialog(null, "Password confirmation doesn't match.", "System Message", JOptionPane.ERROR_MESSAGE);
			return;
		}
		if(employeeModel.changePassword(employeeModel.findByUsername(username).getId(), newPw)) {
			JOptionPane.showMessageDialog(null, "Password has been updated.", "System Message", JOptionPane.INFORMATION_MESSAGE);
			this.setVisible(false);
		}
	}
}
