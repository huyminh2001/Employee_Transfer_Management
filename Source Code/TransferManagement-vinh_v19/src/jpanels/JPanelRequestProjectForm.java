...
public class JPanelRequestProjectForm extends JPanel {
	 ...
	 * Create the panel.
	 */
	public JPanelRequestProjectForm() {
		...
		JLabel lblNewLabel = new JLabel("Request Project Form");
		...
		add(jtextFieldFromProject, gbc_jtextFieldFromProject);
		...
		add(jcomboBoxToProject, gbc_jcomboBoxToProject);
...
//      g2d.drawRoundRect(0, 0, getWidth(), getHeight(), cornerRadius, cornerRadius);
	...
	public JPanelRequestProjectForm(Map<String, Object> data) {
		this();
		this.data = data;
	...
	private void requestProjectRender() {
		EmployeeModel employeeModel = new EmployeeModel();
		ProjectsModel projectsModel = new ProjectsModel();
		String employeeName = data.get("name").toString();
		jtextFieldEmployeeName.setText(employeeName);
		
		String fromProject = String.valueOf(projectsModel.findById(Integer.parseInt(data.get("projectId").toString())).getName());
		jtextFieldFromProject.setText(fromProject);
	...
	private void jcomboBoxProjectTo() {
		...
		int currentEmployeeProject = employeeModel.getCurrentProject(Integer.parseInt(data.get("id").toString()));
		System.out.println(currentEmployeeProject);
...
//			System.out.println(project.toString());
	...
	protected void do_jbuttonRequest_actionPerformed(ActionEvent e) {
		...
		int employeeId = Integer.parseInt(data.get("id").toString());
		int projectFrom = Integer.parseInt(data.get("projectId").toString());
		...
		record.setProjectFrom(projectFrom);
		record.setProjectTo(projectTo.getId());
		...
		String var_projectFromName = projectsModel.findById(record.getProjectFrom()).getName();
		String var_projectToName = projectsModel.findById(record.getProjectTo()).getName();
		...
		if(!recordModel.checkAvailablePendingProjectRecord(requestFor)){
			int result = JOptionPane.showConfirmDialog(null, "Are you sure?", "Comfirm", JOptionPane.YES_NO_OPTION);
			if(result == JOptionPane.YES_OPTION) {
				isCreated = recordModel.createProjectRecord(record);
				MailHelpers.send("vinhduc0304@gmail.com", mailTo, mailCc, mailSubject, mailBody);
			}
		}
		
		String message = isCreated ? "Create Record success" : "Create Record failed. " + requestFor.getName() + " can only have 1 pending project request.";
